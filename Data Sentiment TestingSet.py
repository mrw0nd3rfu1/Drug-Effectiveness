...
X.drop(['patient_id','name_of_drug','use_case_for_drug','drug_approved_by_UIC','number_of_times_prescribed','effectiveness_rating'], axis=1, inplace=True)

temp.drop(['drug_approved_by_UIC'], axis=1, inplace=True)
...
output = pd.DataFrame({'patient_id':temp.patient_id,'name_of_drug': temp.name_of_drug,'use_case_for_drug':temp.use_case_for_drug,'effectiveness_rating':temp.effectiveness_rating,'number_of_times_prescribed':temp.number_of_times_prescribed,'positive':X.positive,'negative':X.negative,'neutral':X.neutral,'compound':X.compound})
